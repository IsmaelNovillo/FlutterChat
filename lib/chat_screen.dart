              ...
              builder: (ctx, AsyncSnapshot<QuerySnapshot> chatSnapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (ctx, index) {
                    final message = chatDocs[index]['text'];
                    if (message.startsWith('Location:')) {
                      final locationUrl = message.substring('Location: '.length);
                      return ListTile(
                        title: GestureDetector(
                          child: Text.rich(
                            TextSpan(
          ...
          Padding(
            ...
            child: Row(
              children: [
                Expanded(
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      filled: true,
                      fillColor: Colors.grey[800] ?? Colors.grey,
                ...
                IconButton(
                  icon: Icon(Icons.send, color: Colors.grey[600] ?? Colors.grey),
                  onPressed: () async {
                    final user = FirebaseAuth.instance.currentUser;
                    if (user != null) {
                      await FirebaseFirestore.instance
                ...
                IconButton(
                  icon: Icon(Icons.location_on, color: Colors.grey[600] ?? Colors.grey),
                  onPressed: () async {
                    final user = FirebaseAuth.instance.currentUser;
                    if (user != null) {
                      Position position = await Geolocator.getCurrentPosition(
